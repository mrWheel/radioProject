...
12. ICY `StreamTitle` metadata is split exactly once, in `main/app_main.c` (`split_icy_title()` inside `title_cb`), on the conventional `"Artist - Track"` separator. Either half — or both, e.g. when a station sends no ICY metadata at all — falls back to a single dash `-`, never an empty string, so neither the display nor the web GUI has to re-decide that fallback. Both consumers receive `(artist, track)` (`radio_display_now_playing()`, `web_gui_notify_title()`), render Artist and Track as two separate, vertically stacked fields, and only redraw/rebroadcast when at least one of the two actually changed since the last value. On the physical TFT display these two lines are horizontally centered and drawn at a larger font scale (`2`, up from the rest of the Volume screen's `1`) than surrounding text.
...
15. Two independent, complementary reactions to network trouble live in `radio_audio.c`:
    ...
    - **"Stream stalled" indicator**: `radio_audio_set_stall_callback()` fires `(stalled=true, ctx)` whenever `fetch_task()` ends *without* a deliberate stop/switch (`s_stop_requested` still false) — i.e. the connection never opened after all retries, an HTTP redirect could not be resolved, an ICY/stream read failed outright, or a post-stall reconnect itself failed. `app_main.c`'s `stall_cb()` reacts by showing the centered placeholder text `"Stream stalled"` in the Artist field on both the TFT (`radio_display_now_playing()`) and the web GUI (`web_gui_notify_title()`), the same centered-Artist-field convention used for `"Switch Station .."`. The callback fires `(stalled=false, ctx)` at the top of every fresh `fetch_task()` run to clear a stale indicator before a new attempt.
...
- [x] Add `radio_audio_set_stall_callback()` so an unexpected stream end shows a centered `"Stream stalled"` placeholder in the Artist field on both the TFT and the web GUI instead of leaving stale text (see requirement 15).
