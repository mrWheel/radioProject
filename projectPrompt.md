...
17. Browser audio is an optional, purely browser-local extra output next to the device; there is no audio-ownership handoff anymore.
    ...
    - The web GUI shows a "Muziek via browser" on/off switch (`browserAudioToggle` in `littlefs/index.html`), rendered as a normal toggle switch, not a slider, and separate from the volume slider. Every page load starts with the switch Off.
