...
17. Browser/device audio ownership is a first-class project rule and is the authoritative control model for the embedded radio.
    - The ESP32 is the single source of truth for the current audio owner. The runtime enum is:
      ```c
      typedef enum
      {
        AUDIO_OWNER_DEVICE,
      ...
      ```
      and the equivalent WebGUI enum is `WEB_GUI_AUDIO_OWNER_DEVICE` / `WEB_GUI_AUDIO_OWNER_BROWSER`.
    - Default behaviour is `AUDIO_OWNER_DEVICE`. A browser must never claim ownership simply because it is open or reconnects. Ownership changes only through an explicit browser action (`audioOwnerSet`) or a device-side takeover event.
    - The browser toggle in the WebGUI is a separate audio-output switch, not the volume control. It is a user-visible selector that must appear as a normal on/off switch, not a slider. The browser state is synchronized through the WebSocket `audioOwner`/`audioOwnerSet` flow, and the browser must also reflect the current owner in its initial state payload and in any subsequent updates.
    - When browser ownership is selected, the device must stop its current I2S stream immediately; `radio_audio` must be stopped or muted and the HTTP connection must be closed so the ESP32 does not continue to produce audio while the browser is active. During the browser ownership window, no audio is produced through the PCM5102A path.
    - When the user switches the control back to Device, the browser must stop its direct HTML5 audio stream, the browser must send the corresponding ownership update to the ESP32, and the actual station must be restarted on the ESP32 through the normal `radio_audio` path. There must be no stale stream or leftover decoded buffer from the old session.
    - Physical input has priority over browser ownership at all times. Any EC11 rotation, EN push, auxiliary button event, or other normal physical input event must immediately revert ownership to `AUDIO_OWNER_DEVICE`, stop browser playback, and restart the current station on the device before the original physical action continues.
    - WebSocket disconnects while browser ownership is active must automatically revert to device ownership and restart the current station on the ESP32. This must happen through the standard WebGUI/WebSocket lifecycle and callback flow, not through any parallel browser-only system.
    - Station changes while browser ownership is active remain browser-owned: the station index still updates on the ESP32 and is saved exactly like a device-driven change, but the browser keeps playing the selected stream directly and the device does not resume I2S output until another explicit device-priority event or explicit browser-to-device switch occurs.
    - Multiple browser clients must not both own audio. The server tracks the active WebSocket client, rejects takeover without explicit user action, and, on a takeover or disconnect, reverts to device ownership and closes the stale socket cleanly. If a stale browser still has audio, it must be stopped immediately.
    - The WebGUI state JSON must include `audioOwner` and the initial `getState` response must report the actual current owner. The ESP32-side callbacks (`web_gui_set_audio_owner_cb()`, `on_web_gui_audio_owner_changed()`, `on_web_gui_state_applied()`) map the WebGUI owner into the main app's authoritative state and keep the physical TFT/UI synchronized with the browser-driven change.
    - The browser must not auto-claim ownership on reconnect or takeover. The server must only switch to `browser` after an explicit user toggle to browser, and the browser's client-side logic must update the visible switch state and the `owner` field to match the actual server-confirmed state.
    - The implementation is intentionally constrained to the existing architecture: no second parallel websocket system, no second input system, and no duplicate audio paths. The only valid output choices remain Device or Browser.
