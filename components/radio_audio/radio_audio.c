...
static volatile uint32_t s_buf_underruns = 0;
...
static radio_audio_title_cb_t s_title_cb;
static void* s_title_ctx;
static radio_audio_mute_cb_t s_mute_cb;
...
static void log_station_banner(const radio_station_t* station)
{
  ...
  ESP_LOGI(TAG, "Minimum free heap so far: %d bytes",
...
static void log_url_details(const char* url)
{
  ...
  if (scheme[0])
    ESP_LOGI(TAG_HTTP, "Scheme   : %s", scheme);
  if (host[0])
    ESP_LOGI(TAG_HTTP, "Host     : %s", host);
  if (port[0])
    ESP_LOGI(TAG_HTTP, "Port     : %s", port);
  else if (strcmp(scheme, "https") == 0)
    ESP_LOGI(TAG_HTTP, "Port     : 443");
  else if (strcmp(scheme, "http") == 0)
    ESP_LOGI(TAG_HTTP, "Port     : 80");
  if (path[0])
    ESP_LOGI(TAG_HTTP, "Path     : %s", path);
  if (strstr(url, ".m3u") || strstr(url, ".pls") || strstr(url, ".m3u8") || strstr(url, ".xspf") ||
      strstr(url, ".asx") || strstr(url, "playlist"))
  {
...
static void log_dns_resolution(const char* host)
{
  ...
  for (size_t i = 0; i < sizeof(families) / sizeof(families[0]); ++i)
  {
    ...
    for (struct addrinfo* ai = result; ai != NULL; ai = ai->ai_next)
    {
      ...
      if (!addr)
        continue;
      if (inet_ntop(ai->ai_family, addr, ipbuf, sizeof(ipbuf)) == NULL)
        continue;
...
static void log_http_response(esp_http_client_handle_t h, int* out_icy_br_kbps)
{
  ...
  if (esp_http_client_get_response_header(h, "Content-Type", &content_type) == ESP_OK &&
      content_type)
  {
  ...
  if (esp_http_client_get_response_header(h, "Transfer-Encoding", &transfer_encoding) == ESP_OK &&
      transfer_encoding)
  {
  ...
  if (esp_http_client_get_response_header(h, "Content-Length", &content_length) == ESP_OK &&
      content_length)
  {
...
static const char* detect_payload_type(const uint8_t* buf, size_t len, const char* content_type)
{
  if (!buf || len == 0)
    return "unknown";
  if (content_type &&
      (strstr(content_type, "text/html") || strstr(content_type, "application/json") ||
       strstr(content_type, "application/xml") || strstr(content_type, "text/xml")))
  {
    return "web-or-data-page";
  }
  if (content_type && (strstr(content_type, "mpegurl") || strstr(content_type, "mpd") ||
                       strstr(content_type, "dash")))
  {
    return "HLS/DASH playlist";
  }
  if (len >= 7 && memcmp(buf, "#EXTM3U", 7) == 0)
    return "HLS/M3U";
  if (len >= 9 && memcmp(buf, "[playlist]", 9) == 0)
    return "PLS";
  if (len >= 5 && memcmp(buf, "<!DOC", 5) == 0)
    return "HTML";
  if (len >= 5 && (memcmp(buf, "<html", 5) == 0 || memcmp(buf, "<?xml", 5) == 0))
    return "HTML/XML";
  if (len >= 4 && memcmp(buf, "OggS", 4) == 0)
    return "Ogg";
  if (len >= 4 && memcmp(buf, "fLaC", 4) == 0)
    return "FLAC";
  if (len >= 3 && (buf[0] == 0xFF && (buf[1] == 0xFB || buf[1] == 0xF3 || buf[1] == 0xF2)))
    return "MP3";
  if (len >= 2 && (buf[0] == 0xFF && (buf[1] == 0xF1 || buf[1] == 0xF9)))
    return "AAC";
  if (len >= 11 && strncmp((const char*)buf, "StreamTitle", 11) == 0)
    return "ICY metadata";
  return "unknown";
}
...
static void log_payload_probe(const uint8_t* buf, size_t len, const char* content_type)
{
  ...
  if (content_type && (strstr(content_type, "mpegurl") || strstr(content_type, "dash") ||
                       strstr(content_type, "mpd") || strstr(content_type, "m3u")))
  {
...
static void log_stream_summary(const char* station_name, const char* final_url, const char* reason,
                               const char* content_type, bool success, int http_code)
{
  ESP_LOGI(TAG, "============================================================");
  if (success)
  {
...
static void log_retry_event(int attempt, int max_attempts, const char* url, const char* reason)
{
  ...
  if (reason)
    ESP_LOGW(TAG_HTTP, "Retry reason: %s", reason);
}

static void log_teardown_summary(const radio_station_t* station, const char* final_url,
                                 const char* reason, size_t bytes_received, int http_code,
                                 uint64_t elapsed_us)
{
  ...
  if (reason)
    ESP_LOGW(TAG, "Last error    : %s", reason);
}
...
static esp_http_client_handle_t create_http_client(const char* url, bool verify_tls)
{
  esp_http_client_config_t hc = {.url = url,
                                 .crt_bundle_attach = verify_tls ? esp_crt_bundle_attach : NULL,
                                 .timeout_ms = 10000,
                                 .buffer_size = 4096,
                                 .keep_alive_enable = true,
                                 .event_handler = http_event_handler,
                                 .user_agent = "ESP32-Internet-Radio/1.0",
...
static esp_http_client_handle_t open_with_retries(const char* url, uint32_t session_id,
                                                  bool* out_tls_verified, char* reason_out,
                                                  size_t reason_cap)
{
  ...
  for (int attempt = 1; attempt <= total_attempts; ++attempt)
  {
    ...
    if (esp_http_client_open(h, 0) == ESP_OK)
    {
      ...
      if (is_https && !tls_verified)
        ESP_LOGW(TAG_TLS, "Connected without TLS certificate verification (bundle rejected "
    ...
    {
      ESP_LOGW(TAG_TLS,
               "Certificate verification failed %d times for %s; falling back to one unverified "
               "TLS attempt",
               attempt, url);
      esp_http_client_cleanup(h);
      tls_verified = false;
    ...
    if (attempt < total_attempts)
    {
      ...
      for (int wait = 0; wait < 20 && !s_stop_requested && session_is_active(session_id); ++wait)
    ...
    if (err != 0)
      ESP_LOGE(TAG_HTTP, "errno=%d (%s)", err, strerror(err));
...
reopen_stream:;
  ...
  for (int redirect = 0; redirect <= max_redirects; ++redirect)
  {
    ...
    if (host)
    {
      ...
      if (host_end)
      {
        ...
        if (len >= sizeof(host_copy))
          len = sizeof(host_copy) - 1;
    ...
    h = open_with_retries(current_url, session.session_id, &tls_verified, failure_reason_buf,
    ...
    if (redirect == max_redirects ||
        esp_http_client_get_response_header(h, "Location", &location) != ESP_OK || !location)
    {
      ESP_LOGE(TAG_HTTP, "Unresolved HTTP redirect (status %d) for %s", http_code, current_url);
      log_stream_summary(station.name, current_url, "unresolved HTTP redirect", NULL, false,
                         http_code);
      esp_http_client_close(h);
      esp_http_client_cleanup(h);
      h = NULL;
  ...
  size_t meta_int = 0;
  char* meta_hdr = NULL;
  if (esp_http_client_get_response_header(h, "icy-metaint", &meta_hdr) == ESP_OK && meta_hdr)
    meta_int = (size_t)atoi(meta_hdr);
  ESP_LOGI(TAG_STREAM, "icy-metaint=%d", (int)meta_int);
  uint8_t* meta_buf = meta_int ? malloc(255 * 16 + 1) : NULL;
  if (meta_int && !meta_buf)
    meta_int = 0;
  ...
  while (!s_stop_requested && session_is_active(session.session_id))
  {
    ...
    if (meta_int && meta_remain == 0)
    {
      ...
      if (meta_len)
      {
        ...
        if (title[0] && session_is_active(session.session_id) && s_title_cb)
          s_title_cb(title, s_title_ctx);
    ...
    if (n <= 0)
    {
      ESP_LOGE(TAG_STREAM, "Stream read failed: n=%d errno=%d (%s)", n, errno,
    ...
    if (avail < s_buf_min_fill)
      s_buf_min_fill = avail;
    uint64_t now_us = esp_timer_get_time();
    if (now_us - s_buf_last_log_us >= BUF_LOG_INTERVAL_US)
    {
      s_buf_last_log_us = now_us;
      ESP_LOGD(TAG_BUFFER, "AUDIO_BUFFER: fill=%zu / %d, min=%zu, underruns=%lu", avail,
               STREAM_BUF_CAPACITY, s_buf_min_fill, (unsigned long)s_buf_underruns);
      uint64_t measured_kbps = read_elapsed_us ? ((uint64_t)n * 8000ULL) / read_elapsed_us : 0;
      if (s_icy_br_kbps > 0)
      {
        ESP_LOGD(
            TAG_HTTP,
            "HTTP_READ: bytes=%d, elapsed=%llu ms, rate=%llu kbps (station advertises %d kbps)", n,
            (unsigned long long)(read_elapsed_us / 1000ULL), (unsigned long long)measured_kbps,
            s_icy_br_kbps);
      }
      else
      {
        ESP_LOGD(TAG_HTTP, "HTTP_READ: bytes=%d, elapsed=%llu ms, rate=%llu kbps", n,
                 (unsigned long long)(read_elapsed_us / 1000ULL),
                 (unsigned long long)measured_kbps);
      }
    }
  }
  free(chunk);
  ...
  {
    log_teardown_summary(&station, current_url, failure_reason_buf, bytes_received, http_code,
...
static void stream_task(void* arg)
{
  ...
  ESP_LOGD(TAG_BUFFER, "AUDIO_BUFFER: prebuffered %zu / %d bytes in %llu ms",
           xStreamBufferBytesAvailable(s_sb), STREAM_BUF_CAPACITY,
           (unsigned long long)((esp_timer_get_time() - prebuf_start_us) / 1000ULL));
  ...
  while (!s_stop_requested && session_is_active(session.session_id))
  {
    ...
    if (n == 0)
    {
      if (s_stop_requested || !session_is_active(session.session_id))
        break;
    ...
    if (raw.len && raw.buffer != in)
      memmove(in, raw.buffer, raw.len);
    in_len = raw.len;
    if (in_len >= in_cap)
      in_len = 0;
  }
...
static void audio_task(void* arg)
{
  ...
  for (;;)
  {
    ...
    *stream_session =
        (audio_session_t){.station = message.station, .session_id = message.session_id};
    *fetch_session = *stream_session;
    ...
    if (xTaskCreatePinnedToCore(stream_task, "radio_stream", 12288, stream_session, 7,
    ...
    if (xTaskCreatePinnedToCore(fetch_task, "radio_fetch", 6144, fetch_session, 6, &s_fetch_task,
                                0) != pdPASS)
    {
      free(fetch_session);
      s_fetch_running = false;
...
esp_err_t radio_audio_init(void)
{
  ...
  ESP_LOGI(TAG, "PSRAM free before ring buffer alloc: %zu bytes",
           heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
  s_sb_storage = heap_caps_malloc(STREAM_BUF_CAPACITY + 1, MALLOC_CAP_SPIRAM);
  if (!s_sb_storage)
    return ESP_ERR_NO_MEM;
  ESP_LOGI(TAG, "Ring buffer allocated: %d bytes in PSRAM (%zu bytes free after)",
           STREAM_BUF_CAPACITY, heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
  ...
  if (!s_queue)
    return ESP_ERR_NO_MEM;
  return xTaskCreate(audio_task, "radio_audio", 4096, NULL, 6, NULL) == pdPASS ? ESP_OK
                                                                               : ESP_ERR_NO_MEM;
}
esp_err_t radio_audio_play(const radio_station_t* s)
{
  if (!s || !s_queue)
    return ESP_ERR_INVALID_ARG;
  s_muted = true;
  s_paused = false;
  if (s_mute_cb)
    s_mute_cb(true, s_mute_ctx);
  uint32_t session_id = s_active_session_id + 1;
...
//-- Ring-buffer fill as a 0-100 percentage so UI components can draw a live
//-- buffer-level indicator without knowing the internal capacity
int radio_audio_get_buffer_fill_percent(void)
{
  if (!s_sb)
    return 0;
  size_t avail = xStreamBufferBytesAvailable(s_sb);
  return (int)((avail * 100) / STREAM_BUF_CAPACITY);
}
