...
bool station_store_valid(const radio_station_t* station)
{
  ...
  if (station->name[0] == '\0' || station->url[0] == '\0')
    return false;
  if (strlen(station->name) >= sizeof(station->name))
    return false;
  if (strlen(station->url) >= sizeof(station->url))
    return false;
  if (strncasecmp(station->url, "http://", 7) != 0 && strncasecmp(station->url, "https://", 8) != 0)
    return false;
  return true;
}
...
esp_err_t station_store_load(char* err_msg, size_t err_msg_size)
{
  ...
  {
    if (err_msg)
      snprintf(err_msg, err_msg_size, "stations.json not found");
  ...
  if (n <= 0 || n > 32768)
  {
    fclose(f);
    if (err_msg)
      snprintf(err_msg, err_msg_size, "stations.json invalid size (%ld bytes)", n);
  ...
  if (!buf)
  {
    fclose(f);
    if (err_msg)
      snprintf(err_msg, err_msg_size, "Out of memory loading stations.json");
  ...
  if (fread(buf, 1, (size_t)n, f) != (size_t)n)
  {
    ...
    fclose(f);
    if (err_msg)
      snprintf(err_msg, err_msg_size, "Failed to read stations.json");
    return ESP_FAIL;
  }
  fclose(f);
  buf[n] = '\0';
  ...
  if (!root)
  {
    ...
    {
      int line = 1;
      for (const char* p = buf; p < err_ptr; ++p)
      {
        if (*p == '\n')
          line++;
      }
